 ...
 * <p> Description: [目前此方法不用修改，实现{@link IColumnDetail}接口的所有类都会作为导出的一列，所以请关注{@link IColumnDetail}接口实现类]</p>
 ...
 * @version 1.0
 */
public class ExportDetailButtonCommand implements IButtonCommand {
	...
	public Info handleRequest() throws BizException, Exception {
		...
		if (info.getRecordBeanList() == null) {
			...
			List<Class<?>> lc = ClassUtil.getAllImplClassesByInterface(IColumnDetail.class);
			List<Map<String, String>> lm = Business.generateEmployeeDetailList(info.getEmployeeBeanList(),
					info.getRecordBeanList(), info.getEvectionBeanList(), lc);
