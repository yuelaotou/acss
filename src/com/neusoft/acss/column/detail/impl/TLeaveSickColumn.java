...
public class TLeaveSickColumn implements IColumnDetail {
	...
	public String generateColumn(Info info) {
		...
		if (evb != null) {
			...
			if (StringUtils.isNotEmpty(evb.getLeave_sick())) {
				if (evb.getLeave_sick().equals("全天")) {
					return "8";
				} else if (evb.getLeave_sick().equals("上午")) {
					return "4";
				} else if (evb.getLeave_sick().equals("下午")) {
					return "4";
				} else {
					String beforetime = StringUtils.substringBefore(evb.getLeave_sick().replace("-", "~"), "~");
					String endtime = StringUtils.substringAfter(evb.getLeave_sick().replace("-", "~"), "~");
